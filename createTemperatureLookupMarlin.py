...
class Thermistor:
    ...
    def r(self,adc):
        ...
        vr = adc * self.vadc / (1024 )   # convert the 10 bit ADC value to a voltage
        vt = self.vcc - vr
        rt = self.rp * vt / vr    # resistance of thermistor
        return rt
	...
	x = sqrt(pow(self.c2 / (3*self.c3),3) + pow(y,2))
        rt = exp(pow(x-y,1.0/3) - pow(x+y,1.0/3)) # resistance of thermistor
        return (self.rp / (self.rp + rt)) * (1024)
